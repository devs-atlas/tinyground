...
export default class Tensor {
  ...
  expand(shape) {
    //TODO: bug here - overindexing shape
    const expandedShape = shape.map((x, i) => (x == -1 ? this.shape[i] : x));
    console.log("Original shape:", this.shape);
    console.log("Requested shape:", shape);
    console.log("Expanded shape:", expandedShape);

    // Run the operation
    const result = mlops.Expand.run_op([this], expandedShape);
