...
import * as mlops from "./mlops.js";

//TODO: look into tidy and dispose - necessary somewhere fs
//TODO: think about making sigmoid more numerically stable???
//TODO: dotting vector and vector results in [] shape... problem?
//TODO: switch from tfjs broadcasting to y
...
export default class Tensor {
  ...
  get array() {
    return this.data.data.arraySync();
  }
  ...
  get scalar() {
    return (this.shape.reduce((acc, x) => (acc *= x)) === 1) || this.shape.length == 0;
  }
  ...
  dot(w) {
    ...
    if (!x.vector && !w.vector) {
      ...
      w = w.reshape(newWShape).transpose(newWShape.length - 1, newWShape.length - 2);
    }
    if (x.vector && !w.vector) {
      w = w.transpose(w.shape.length - 1, w.shape.length - 2);
    }
    return (x.mul(w)).sum(-1);
  }

  _broadcasted(y, reverse = false) {
    let x = this;

    if (typeof (y) == number) return x, x.full_like(y);
  ...
  _reduce(op, axis, keepdim = false) {
     ...
     */
    let reducedShape = this.shape.filter((_, i) => !axis_.includes(i));
    ...
    let new_shape = this.shape.map((s, i) => (axis_.includes(i) ? 1 : s));
    ...
    if (keepdim) {
      console.log(`original: ${this.data.data.arraySync()}`);
      console.log(`keepdim: ${ret.reshape(reducedShape).data.data.arraySync()}`);
    }
  ...
  permute(order) {
    if (order.length != this.shape.length) {
      throw new Error("Axis orders must match tensor dimension");
    }
  ...
  expand(shape) {
    if (this.shape.length != shape.length) {
      throw new Error("Number of dims of expanded shape must be same as this.shape");
    }
    const expandedShape = shape.map((x, i) => (x == -1 ? this.shape[i] : x));
    const result = mlops.Expand.run_op([this], { shape: expandedShape });
